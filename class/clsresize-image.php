...
class resize_image {
  ...
  function save_image($im, $filename) {
  ...
  function image_create_from_type($type,$filename) {
   $im = null;
   switch ($type) {
     case 1:
  ...
  function generate_thumb($from_name, $to_name) {
    ...
    if ($orig_x > $this->max_x or $orig_y > $this->max_y) {
	  ...
	  $file_info = pathinfo($from_name);
      return trim($file_info['basename']);
    }
    ...
    $im = $this->image_create_from_type($orig_img_type,$from_name);
    ...
    // save thumb file
    $this->save_image($ni, $to_name);

    if($temp) {
      unlink($tmpfname); // this removes the file
    }

	$file_info = pathinfo($to_name);
	return trim($file_info['basename']);
