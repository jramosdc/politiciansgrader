<?php
...

	{
		...
		public static $template_error_message = array (
														...
														,444 => '<li>&ndash; This ##field_name## is already subscribed with us..</li>'
														
														,44 => '<li>&ndash; This ##field_name## address is already registered with us.</li>'
														...
														,66 => '<li>&ndash; Please enter greater than or equal to ##min_length## characters in ##field_name##.</li>'
														...
														,1000 => '<li>&ndash; ##field_name## must greater than of ##field2_name##.</li>'
					...
					if ( is_array($validation) && count($validation) ) {
							...
							switch ( trim($validation['validation_type']) ) {
								...
								case 'compare': //compare two controls
									...
									if ( trim($_POST[$strcontrol_name]) != trim($_POST[$compare_with]) ) {
								...
								case 'greater':
									...
									if ( (int) trim($_POST[$strcontrol_name]) <= (int) trim($_POST[$compare_with]) ) {
										...
										if ( trim($validation['custom_error_message']) != '' ) {
											...
											$error_message = str_replace('##field_name##', ucfirst(common::get_value($validation['control_friendly_name'])), self::$template_error_message[1000]);
								...
								case 2: //only float
									...
									if ( preg_match('/[0-9.]/', trim($_REQUEST[$strcontrol_name])) == false ) {
										...
										self::$error_message[] = str_replace('##field_name##', strtolower(common::get_value($validation['control_friendly_name'])), self::$template_error_message[5]);
								...
								case 'min_length': //min length
									...
									$intmin_length = (int) $validation['extra_args'];

									if ( $intvalue_length < $intmin_length ) {

										self::$error_message[] = str_replace('##min_length##', (string) intval($intmin_length), str_replace('##field_name##', strtolower(common::get_value($validation['control_friendly_name'])), self::$template_error_message[66]));
								...
								case 4: //greater than
									...
									if ( ! (floatval($_REQUEST[$strcontrol_name]) > $compare_with_value) ) {
										...
										self::$error_message[] = str_replace('##field_name##', strtolower(common::get_value($validation['control_friendly_name'])), $strerror_message);									
								...
								case 'duplicate_subscribe': //duplicate validation
										...
										if ( is_array($extra_args) && count($extra_args) ) {
											...
											if ( $rsduplicate_recs && mysql_num_rows($rsduplicate_recs) ) {
												...
												if ( trim($validation['custom_error_message']) != '' ) {
													...
													self::$error_message[] = str_replace('##field_name##', strtolower(common::get_value($validation['control_friendly_name'])), self::$template_error_message[444]);
								...
								case 'email': //valid email
									...
									$strfield_value = trim($_POST[$strcontrol_name]);
								...
								case 'duplicate_email': //duplicate email validation
										...
										if ( is_array($extra_args) && count($extra_args) ) {
											...
											if ( $rsduplicate_recs && mysql_num_rows($rsduplicate_recs) ) {
												...
												if ( trim($validation['custom_error_message']) != '' ) {
													...
													self::$error_message[] = str_replace('##field_name##', strtolower(common::get_value($validation['control_friendly_name'])), self::$template_error_message[44]);
								...
								case 'captcha': //required field
									...
									//echo $_POST[$strcontrol_name];
									//echo $_SESSION['captcha_code'];
									if ( trim($_POST[$strcontrol_name]) != $_SESSION['captcha_code'] ) {
